        ...
        <div class="card container-fluid mb-4 p-6">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <small class="text-muted float-end">
                    <a href="{{ route('blog.index') }}" class="btn btn-sm btn-primary"><i class="fa-solid fa-arrow-left"></i>
                        Back</a>
                </small>
            </div>
                            ...
                            @foreach($seo as $name => $label)
                                <div class="form-group mb-3">
                                    ...
                                    @if($name == 'seo_description' || $name == 'seo_schema')
                                        <textarea name="{{ $name }}" id="{{ $name }}" rows="{{ $name == 'seo_schema' ? 6 : 3 }}" class="form-control br-8 @error($name) is-invalid @enderror" placeholder="Enter {{ strtolower($label) }}">{{ old($name) }}</textarea>
                                    @else
                                        <input type="text" name="{{ $name }}" value="{{ old($name) }}" class="form-control br-8 @error($name) is-invalid @enderror" placeholder="Enter {{ strtolower($label) }}">
                                    @endif
                                    @error($name)
